...
def seed_db():
    ...
    mika = User(
        {"username": "mika_test", "password": generate_password_hash("mikamika")}
    )
    zika = User(
        {"username": "zika_test", "password": generate_password_hash("zikazika")}
    )
    kiki = User(
        {"username": "kiki_test", "password": generate_password_hash("kikikiki")}
    )
...
class TestClassLogin:
    ...
    def test_login_success(self, client: FlaskClient):
        response = client.post(
            "/api/login", json={"username": "pera_test", "password": "perapera"}
        )
        assert response.status_code == 200
    ...
    def test_login_without_password(self, client: FlaskClient):
        ...
        response = client.post(
            "/api/login", json={"username": incoming_data["username"]}
        )
        assert response.status_code == 400
