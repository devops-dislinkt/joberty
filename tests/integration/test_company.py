...
class TestCompany:
    ...
    def test_create_company_request_success(
        self, client: FlaskClient, valid_co_data: dict
    ):
        response = client.post(
            "/api/company", json=valid_co_data, headers=self.get_headers_valid(mika)
        )
        assert response.status_code == 200
        assert response.json["approved"] == False

    def test_create_company_request_without_login(
        self, client: FlaskClient, valid_co_data: dict
    ):
    ...
    def test_create_company_request_with_invalid_data(
        self, client: FlaskClient, invalid_co_data: dict
    ):
        """Request should fail if invalid data."""
        response = client.post(
            "/api/company", json=invalid_co_data, headers=self.get_headers_valid(mika)
        )
    ...
    def test_reject_company_registration(self, client: FlaskClient):
        ...
        response = client.post(
            "/api/company/resolve-registration",
            json=incoming_data,
            headers=self.get_headers_valid(admin),
        )
        assert response.status_code == 200

    def test_accept_company_registration(
        self, client: FlaskClient, valid_co_data: dict
    ):
        """Only admin can resolve company registration. When admin accepts, company is approved and user becomes the owner."""
        # make request first
        req_res = client.post(
            "/api/company", json=valid_co_data, headers=self.get_headers_valid(zika)
        )
    ...
    def test_accept_company_registration_with_wrong_role(
        self, client: FlaskClient, valid_co_data: dict
    ):
        """Only admin can resolve company registration. When admin accepts, company is approved and user becomes the owner."""
        # make request first
        ...
        response = client.post(
            "/api/company/resolve-registration",
            json=incoming_data,
            headers=self.get_headers_valid(zika),
        )
        assert response.status_code == 403
    ...
    def test_get_all_approved_companies(self, client: FlaskClient):
        """Should return all companies"""
        response = client.get(
            "/api/company/approved", headers=self.get_headers_valid(admin)
        )
        assert response.status_code == 200
    ...
    def test_get_all_not_resolved_companies(self, client: FlaskClient):
        """Should return all companies"""
        response = client.get(
            "/api/company/not-resolved", headers=self.get_headers_valid(admin)
        )
        assert response.status_code == 200
        assert 1 == len(response.json)
    ...
    def test_create_comment_success(self, client: FlaskClient):
        ...
        company_id = 3
        response = client.post(
            f"/api/company/{company_id}/comment",
    ...
    def test_add_grade(self, client: FlaskClient):
        ...
        company_id = 3
        response = client.post(
            f"/api/company/{company_id}/grade",
            json={"grade": grade},
            headers=self.get_headers_valid(mika),
        )
        assert response.status_code == 200
    ...
    def test_get_company_success(self, client: FlaskClient):
        company_id = 3
        response = client.get(
            f"/api/company/{company_id}", headers=self.get_headers_valid(mika)
        )
        assert response.status_code == 200
