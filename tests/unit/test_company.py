...
class TestCompany:
    ...
    def test_create_company_request_success(
        self, app: Flask, mika: User, valid_co_data: dict
    ):
        company = company_service.create_company_registration(
            user=mika, data=valid_co_data
        )
    ...
    def test_create_company_request_with_approved_set_to_true(
        self, app: Flask, mika: User, valid_co_data: dict
    ):
    ...
    def test_create_company_request_invalid_data(
        self, app: Flask, mika: User, invalid_co_data: dict
    ):
    ...
    def test_reject_company_registration(self, app: Flask, mika: User):
        ...
        res = company_service.resolve_company_registration(
            username="mika_test", reject=True
        )
        assert res == True
    ...
    def test_accept_company_registration(self, app: Flask, mika: User):
        ...
        company = company_service.resolve_company_registration(
            username="mika_test", reject=False
        )
        assert company != None
    ...
    def test_create_comment_success(self, app: Flask, mika: User, zika: User):
        ...
        comment = company_service.create_comment(
            user=mika, company_id=company.id, description=description
        )
    ...
    def test_create_comment_for_non_existing_comapny(self, app: Flask, mika: User):
        ...
        with pytest.raises(NoResultFound):
            company_service.create_comment(
                user=mika, company_id=100, description=description
            )
    ...
    def test_create_comment_as_company_owner(self, app: Flask, zika: User):
        ...
        with pytest.raises(Exception):
            company_service.create_comment(
                user=zika, company_id=company.id, description=description
            )
    ...
    def test_grade_company_success(self, app: Flask, mika: User, zika: User):
        ...
        grade = 5
        company = zika.company
        ret_grade = company_service.add_grade(
            user=mika, company_id=company.id, grade=grade
        )
